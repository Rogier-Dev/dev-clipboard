...
This runs the Risk rules, clip classification/title helpers, and search metadata tests without adding a separate test runner dependency.

For only the Risk rule subset:
...
- `src/clipModel.ts` — clipboard vault/type/title/count helpers extracted from the UI for tests.
- `src/searchModel.ts` — search-index metadata and developer synonym helpers extracted from the UI for tests.
