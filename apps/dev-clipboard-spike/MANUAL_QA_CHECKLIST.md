...
| [~] | Long token | 長いSSH公開鍵風文字列を表示 | 横にはみ出さず、読める範囲で折り返す | 実キーは使わない。2026-07-15 タイトル、コード、プレビュー、ノート、編集textarea、タグのCSS制約を追加。実機表示確認が残る |
| [~] | Japanese wrap | 長い日本語タイトル | カード内で自然に折り返す | 2026-07-15 タイトル、ノート、プレビューの折り返しCSSを追加。実機表示確認が残る |
